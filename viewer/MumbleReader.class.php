...
//    $base = $ICE->stringToProxy('Meta:tcp -h '. $_SESSION['mumip'] .' -p 6502')->ice_context($_SESSION['icepass']);
//   return Murmur_MetaPrxHelper::checkedCast->$base;
  ...
  private function loadData() {
    ...
    foreach ($servers as $id => $iceServer) {
      if($iceServer->ice_context($_SESSION['icepass'])->getConf('port') == $this->port) {
        $port       = $iceServer->ice_context($_SESSION['icepass'])->getConf('port');
        $servername = $iceServer->ice_context($_SESSION['icepass'])->getConf("registername");
        $tree       = $iceServer->ice_context($_SESSION['icepass'])->getTree();
        $uptime       = $iceServer->ice_context($_SESSION['icepass'])->getUptime();
